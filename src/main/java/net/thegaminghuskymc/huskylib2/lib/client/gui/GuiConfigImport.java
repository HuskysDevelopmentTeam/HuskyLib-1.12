...
public class GuiConfigImport extends GuiConfigBase {
	...
	public void initGui() {
		...
		title += " - " + I18n.translateToLocal("hl2.config.import");

		int x = width / 2 - 100;
		int y = height / 6;
		
		GuiButton importButton = new GuiButton(1, x, y + 110, 200, 20, I18n.translateToLocal("hl2.config.import"));
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		case 1: // Import/Quit
			if(!needsRestart) {
				...
				if(needsRestart) {
					button.displayString = I18n.translateToLocal("hl2.config.close");
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		for(int i = 0; i < 4; i++) {
			int y = 50 + i * 10;
			String key = "hl2.config.importinfo" + i;
			if(i == 3) {
		...
		if(needsRestart) {
			...
			if(disabledFeatures == 1)
				s = I18n.translateToLocalFormatted("hl2.config.disabledcount1");
			else s = I18n.translateToLocalFormatted("hl2.config.disabledcount", disabledFeatures);
			
			drawCenteredString(mc.fontRenderer, s, x, textField.y + 26, 0x00FF00);
		}
		
		if(mc.world != null)
			drawCenteredString(mc.fontRenderer, I18n.translateToLocal("hl2.config.cantimport"), x, textField.y + 26, 0xFF0000);
	}
